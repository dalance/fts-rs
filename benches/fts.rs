...
fn fts_walkdir( b: &mut Bencher ) {
    b.iter( || {
        let mut _cnt = 0;
        for entry in fts::walkdir::WalkDir::new( fts::walkdir::WalkDirConf::new( "/usr" ).no_metadata() ) {
            match entry {
                Ok ( x ) => if x.file_type().is_file() { _cnt += 1 },
                Err( _ ) => (),
            }
        }
        //println!( "{} {}", _cnt );
...
fn fts_walkdir_metadata( b: &mut Bencher ) {
    b.iter( || {
        ...
        for entry in fts::walkdir::WalkDir::new( fts::walkdir::WalkDirConf::new( "/usr" ) ) {
            match entry {
                Ok ( x ) => if x.file_type().is_file() { _cnt += 1; _size += x.metadata().unwrap().len() },
                Err( _ ) => (),
            }
        }
        //println!( "{} {}", _cnt, _size );
